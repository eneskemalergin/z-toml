...
fn roundtrip(gpa: std.mem.Allocator, src: []const u8) !void {
    ...
    // Re-parse and compare tree structure
    var err: toml.ErrorInfo = .{};
...
test "formatter: sort_keys works on nested tables" {
    const gpa = std.testing.allocator;
    // Tables with sub-tables must use [header] syntax, not inline.
    // Each has a non-inline sub-table 'sub' so isInlineTable returns false.
    const src =
        \\[z_last]
        \\z = "last"
        \\a = "first"
...
test "formatter: default emits inline for scalar-only sub-tables" {
    ...
    const out = w.buffered();
    // Default: inline for scalar-only tables
